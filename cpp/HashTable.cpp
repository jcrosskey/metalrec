...
bool HashTable::insertDataset(Dataset* d, UINT64 minOverlapLength)
{
	...
	FILE_LOG(logDEBUG2) << "Orientation: " << (readID >> 62) << endl;
...
bool HashTable::insertIntoTable(Read *read, string subString, UINT64 orientation)
{
	...
	{
		FILE_LOG(logINFO) << currentCollision << " collisions for read " << read->getReadNumber() << " " << subString << " " << orientation;
	}
	return true;
}
