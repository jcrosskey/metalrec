...
def get_bases_from_align(align, start_pos):
    ...
    return pos_dict, ins_dict
## ======================================================================
def read_and_process_sam(samFile,rseq, maxSub=3, maxIns=3, maxDel=3,maxSubRate=0.02, maxInsRate=0.2, maxDelRate=0.2, minPacBioLen=1000, minCV=1,outsam='',outFastaFile=''):
...
def read_and_process_sam_samread(samFile,rseq, maxSub=3, maxIns=3, maxDel=3,maxSubRate=0.02, maxInsRate=0.2, maxDelRate=0.2, minPacBioLen=1000, minCV=1,outsamFile='',outFastaFile=''):
    ''' Get consensus sequence from alignments of short reads to a long read, in the process, filter out bad reads and improve mapping
    ...
    return ref_bps, ref_ins_dict, readinfo
## ======================================================================
def read_and_process_sam_pair(samFile,rseq, maxSub=3, maxIns=3, maxDel=3,maxSubRate=0.02, maxInsRate=0.2, maxDelRate=0.2, minPacBioLen=1000, minCV=1,outsam='',outFastaFile=''):
    ''' Get consensus sequence from alignments of short reads to a long read, in the process, filter out bad reads and improve mapping
...
def ref_extension(poly_bps, poly_ins, consensus_bps, consensus_ins, rseq, region=None, print_width = 100):
    ...
    sys.stdout.write("Extend the PacBio sequence with the insertion positions in the specified region with extended sequence above original sequence.\n")
    sys.stdout.write("X means consensus base call different from original\n")
    sys.stdout.write("Y means positions is polymorphic\n\n")
...
def make_type_array(poly_bps_ext, poly_ins_ext, consensus_bps_ext, consensus_ins_ext):
    ...
    '''
    poly_bps_pos = array( [i[0] for i in poly_bps_ext], dtype=int32 ) # extract positions in the big sequence, for each type
    poly_ins_pos = array( [ i[0] for i in poly_ins_ext], dtype=int32 )
    consensus_bps_pos = array( [i[0] for i in consensus_bps_ext], dtype=int32 )
    consensus_ins_pos = array( [ i[0] for i in consensus_ins_ext], dtype=int32 )
    all_pos = hstack((poly_bps_pos, poly_ins_pos, consensus_bps_pos, consensus_ins_pos)) # all the positions
...
def make_read_array1d(read_string, bp_pos_dict, ins_pos_dict, type_array, poly_bps_ext, poly_ins_ext, consensus_bps_ext, consensus_ins_ext, ext_region=None):
    ''' Make 1d array for a particular read from its string (key of dictionary readinfo).
        ...
        Input:  read_string - read string (key of dictionary readinfo)
                ...
                poly_bps_ext, poly_ins_ext, consensus_bps_ext, consensus_ins_ext - base calling information for all the positions (non-extended sequence)
                ext_region - (start, end) of the region on the extended sequence
...
def greedy_fill_gap(read_array, ref0=None, verbose=False):
    ...
    # Stop whenever the total length of gaps decreases, or none of the gap could be filled at all.
        ...
        if verbose:
            for i in len(gap_lens):
                sys.stdout.write("({}, {}): {}\n".format(gap_start_ind[i], gap_end_ind[i], gap_lens[i]))
        ...
        while not improved and gap_ind < (len(gap_lens) - 1): # until gap length improved, or all the gaps have been investigated
            ...
            while len(best_gap_pos) > 0 and (not totally_filled) and len(reads_ind) > 0 :
                ...
                if sum(gap_lens1) < Min_tot_gap:
                    improved = True
                    Min_tot_gap = sum(gap_lens1) # update current best gap size and the corresponding ref_array
                    best_ref = ref1 # update the reference array
                    best_gap_pos = gap_pos1 # update the gap positions

                # if this step decreased the number of gaps by at least 1, and maximum gap is among them, then stop iteration
                if (len(gap_lens) - len(gap_lens1) >= 1) and gap_start_ind[gap_ind] in setdiff1d(gap_start_ind, gap_start_ind1):
                    totally_filled = True
    ...
    #sys.stdout.write("\n")
        return best_ref, Min_tot_gap, Cvec1
## ======================================================================
def fill_gap(read_array, outFastaFile=None, outDir=None, readinfo=None, verbose=False):
    ...
    while True:
        ...
        #print "ref0:", ref0
        if Mingap > 0:
            ref1 = greedy_fill_gap(read_array, ref0, verbose=verbose)
            gaps = ref1[1]
            if gaps < Mingap:
                Mingap = gaps
                ref0 = ref1
